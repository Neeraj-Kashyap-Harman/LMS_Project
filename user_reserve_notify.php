...
<?php
	...
	include('conf.php');
     ...
     $sql = "SELECT Notification.Not_id,transaction.trans_id,transaction.start_date,transaction.end_date,transaction.type,Notification.status,transaction.u_id,transaction.Book_id,Book.ISBN,Book.Book_Title,Book.Category,Book.Author,Book.No_of_pages,Book.No_of_copies,Book.Available_copies from transaction,Book,Notification where Notification.Book_id=transaction.Book_id and Notification.u_id=transaction.u_id and transaction.Book_id=Book.Book_id and transaction.type = 'reserve' and transaction.u_id=$uid";
...
function Fun_cancel(id,id2,id3,id4) 
{
	...
	if(conf)
	{
	   window.location.href="issue_reserve_cancel.php?bid="+id+"&uid="+id2+"&type=reserve&trans_id="+id3+"&Not_id="+id4;
	}
	else
	{
