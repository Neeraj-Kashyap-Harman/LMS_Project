...
<body>
 ...
 <li class="user-header"></li>
	   ...
	   <form method='get'>
			<input type="text" name="uname" placeholder="Enter   Name" size=35 id="uname">
			
			 <select id="role" name="Role">
          
            <?php
		include('conf.php');
		$sql6 = "select DISTINCT user_role from users";
		$result = mysqli_query($conn,$sql6);
		while($row = mysqli_fetch_array($result))
		{
		echo "<option value=".urlencode($row['user_role']).">";
		echo $row['user_role'];
		echo "</option>";
		}
		echo "<option selected disabled>User Role</option>";
...
<?php
	...
	if(isset($_GET['btn1']))
{
	echo "<a href='admin_userlist.php'><input type='button' value='Back..'></a> ";
	$uname=$_GET['uname'];
	$role=$_GET['Role'];
	if($uname!=NULL && $role==NULL)
		$strSQL = "SELECT * FROM users where Name like '%".$uname."%'";
	if($uname==NULL && $role!=NULL)
		$strSQL = "SELECT * FROM users where user_role='".$role."'";
	if($uname!=NULL && $role!=NULL)
		$strSQL = "SELECT * FROM users where Name like '%".$uname."%' and user_role ='".$role."'";
	...
	if(!isset($_GET['btn1']))
{
	$sql = "SELECT count(u_id) FROM users";
        ...
        $sql2 = "SELECT * from users LIMIT $offset, $rec_limit";
...
echo  "</tbody>
  ...
  if( $page == 0 && $left_rec-$rec_limit>0 )
         {
             echo "<center>";
