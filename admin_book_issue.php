...
<body>
 ...
 <li class="user-header"></li>
 	 ...
 	 <form method='get'>
		...
		include('conf.php');
		$sql6 = "select DISTINCT book_status from Issue";
		$result = mysqli_query($conn,$sql6);
		while($row = mysqli_fetch_array($result))
		{
		echo "<option value=".urlencode($row['book_status']).">";
		echo $row['book_status'];
		echo "</option>";
		}
		echo "<option selected disabled>Filter By</option>";
...
header('Access-Control-Allow-Origin: *');       
	...
	if(isset($_GET['btn1']))
{
	echo "<a href='admin_book_issue.php'><input type='button' value='Back..'></a> ";
	$stat =urldecode($_GET['Status']);
	$uname=$_GET['uname'];
	
	if($uname!=NULL && $stat==NULL)
		$sql = "SELECT * FROM Issue,users where Issue.u_id=users.u_id and Name like '%".$uname."%'";
	if($uname==NULL && $stat!=NULL)
		$sql = "SELECT * FROM Issue where book_status='".$stat."'";
	if($uname!=NULL && $stat!=NULL)
		$sql = "SELECT * FROM Issue,users where Issue.u_id=users.u_id and Name like '%".$uname."%' and book_status ='".$stat."'";
	...
	if($bs == "Returned")
	{
	echo "<input type='button' name='but2' value ='".$row['book_status']."'  style='background-color:lightgreen; width:120px; height:40px' >";
	}
	if($bs == "Not Returned")
	{
	echo "<input type='button' name='but1' value ='".$row['book_status']."'  style='background-color:#F44336; width:120px; height:40px' onclick ='Fun_return($bid,$uid)' >";
	}
	echo "</td><td>";
	echo $row['penalty'];
	echo "</td>";
...
/*******************************************************************************/
	if(!isset($_GET['btn1']))
{
     $sql = "SELECT count(issue_id) FROM Issue";
       ...
       $sql1 = "SELECT * from Issue LIMIT $offset, $rec_limit";
...
$id=$row['issue_id'];
    ...
    $interval = date_diff(date_create(date("Y-m-d")),date_create($row['return_date']));
    
    
    

           
       $penalty=$interval->format('%a');//dateDifference($return , $today );
	...
	echo "</td><td>";
        ...
        mysqli_query($conn,"update Issue set penalty=$penalty*5 where book_status='Not Returned' and issue_id=$id");
